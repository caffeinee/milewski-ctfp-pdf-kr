...
(즉, \trComposition\가능한 쌍에 대해) \trArrow들을 합성한 것을 추가한다.
새 \trArrow\를 추가할 때마다, 그 \trArrow\와 다른 \trArrow들의 \trComposition에 대해서도 생각해야 한다.
...
이러한 \trCategory\는 \trFreeConstruction의 예시이다. \trFreeConstruction이란, 
미완성인 구조가 주어지면 이를 확장하여, 그 구조의 성질을 만족시키면서 최소의 (!!item)을 포함하게 하는 것을 말한다. %item 뭐라 번역할까요, 원소? 내용물?
...
이러한 성질을 만족하는 \trCategory\를 ``\trThin''다고 부른다. \trPreorder\는 \trThin\은 \trCategory다.
...
%%%!!!살려주십시오!!! FRP는 멋대로 번역하겠지만, foldable은 뭐라고 해야 할지 도저히 모르겠어요.
...
mappend s1 s2 = (++) s1 s2
\end{snip}
전자는 \trCategory $\Hask$(끝나지 않는 계산 \trBottom\을 무시하면 $\Set$)에서의 \trMorphism의 동등성으로 해석할 수 있다.
...
%%%proposed syntax...? 제안된 표현법 하면 뉘앙스가 제대로 전해지지 않는 것 같아요ㅠㅠㅠ 검토중인 표현법? 하면 너무 길구...
%%%그리고 concepts는 
...
\code{Monoid}라는 콘셉트는
주어진 타입 \code{M}에 대해 \code{mempty}와 \code{mappend}에 대한 적절한 정의가 존재하는지 테스트하는 \trPredicatesㅣ다.
...
\section{\trCategory\로써의 \trMonoid }

지금까지는 집합의 원소들을 가지고 ``익숙한'' 방식으로 \trMonoid\를 정의해보았다. 
그러나, \trCategoryTheory에서는 집합과 그 원소들이 아니라, \trObject\와 \trMorphism들에 대한 이야기를 한다.
...
즉 \trAdder의 \trComposition\과 덧셈에 적용되는 법칙들은 동치나 다름없다. 역시 좋다. 이제 덧셈을 함수의 \trComposition\로 바꾸어 볼 수 있다.
...
이제, 우리가 지금껏 자연수 집합에서 여러 가지를 해왔다는 사실을 잊고, 대신 이 집합을 \trMorphism\이(\trAdder\가) 많이 달린 \trObject\라고 생각하자.
\trMonoid(monoid)는 단 한 개의 \trObject\를 가진 \trCategory다. 사실 \trMonoid(monoid)라는 단어도 ``하나''를 뜻하는 그리스어 \emph{mono}에서 온 것이다.
모든 \trMonoid\는 단 한 개의 \trObject\와, 적절하게 \trComposition에 관련된 법칙들을 따르는 \trMorphism의 집합으로 설명될 수 있다.
...
이런 질문을 할 수도 있다. 임의의 \trCategory적 \trMonoid(\trObject\가 하나인 \trCategory)로, 집합과 이항연산자가 들은 \trMonoid\를 유일하게 정의할 수 있을까??
...
\caption{\trMorphism\로써의, 그리고 집합 내의 점들로써 \trMonoid\trHomSet\을 볼 수 있다.}
\end{figure}

\noindent
수학자들이 걸고 넘어질 수 있는 사소한 문제가 하나 있다. \trMorphism들이 집합을 이루리라는 보장이 없다는 것이다.
\trCategory의 세계에서는, 집합보다 큰 것들이 존재한다. 임의의 두 \trObject 사이에 존재하는 \trMorphism\이 집합을 이루는 \trCategory\는 \trLocallySmall다고 부른다.
약속한 대로 이런 미묘한 부분들은 무시하고 넘어갈 것이나, 혹시 모르니 일단 언급해둔다.

\trCategoryTheory에서 여러 흥미로운 현상들의 근본에는 \trHomSet의 원소들을 \trComposition의 규칙을 따르는 \trMorphism\으로도, 집합의 점들로도 볼 수 있다는 사실이 있다.
...
\item
  AND 연산자에 대한 \code{Bool} \trMonoid\를 \trCategory\로써 표현하시오. \trMorphism들과 각각의 \trComposition 법칙을 나열하시오.
\item
  법 3(modulo 3)에 대한 덧셈을 \trMonoid \trCategory\로 표현하시오. %법 3이 맞는 표현은 맞는데, 저걸 들었을 때 감이 오시나요?
\end{enumerate}